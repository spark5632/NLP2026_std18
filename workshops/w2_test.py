...
def balance_legal_data(X,y):
    ...
    if min_samples > 1 :
        sampler = SMOTE(k_neighbors=min(5, min_samples-1),random_state=18)
    else:
        sampler = RandomOverSampler(random_state=18)
...
model = LegalBiLSTM()
sample_input = torch.randn(1,5,16) #  1 doc 5 
...
def train_and_evaluate(model_class,name, X,y,Class_names):
    ...
    sns.heatmap(cm,annont=True,fmt="d",cmap="Blues"if"Bi" in name else "Pinks"
...
class_list = ["NO-INF","PATTEIT","COPYRIGHT"]

#X-train,Y-train = ไม่ผ่าน mote , X_res ,y_res = ผ่านการ SMOTE แล้ว
report_lstm = train_and_evaluate(LegalLSTM,"Unidirextional LSTM", X_res_3d,y_res,class_list)
report_bilstm = train_and_evaluate(LegalBiLSTM,"Unidirextional LSTM", X_res_3d,y_res,class_list)
#ตรวจ 
